...
class ReihungComp extends React.PureComponent<Props> {
  ...
  render() {
    ...
    return <div className="Reihung">
        {Boolean(reihung.specificTrainType) && <span className="Reihung__specificType">{reihung.specificTrainType}</span>}
        ...
        <div className="Reihung__reihung">
          <ReihungContext.Provider value={{
          specificType: reihung.specificTrainType,
          type: reihung.zuggattung
        }}>
            {reihung.allFahrzeuggruppe.map(g => <Gruppe specificType={reihung.specificTrainType} type={reihung.zuggattung} showDestination={reihung.differentDestination} key={g.fahrzeuggruppebezeichnung} gruppe={g} />)}
          </ReihungContext.Provider>
        </div>
        <span className={cc(['Reihung__richtung', {
...
export default connect((state: AppState, props: OwnProps): ReduxProps => ({
